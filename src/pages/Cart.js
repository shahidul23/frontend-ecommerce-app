import React from 'react'
import Meta from '../components/Meta'
import BreadCrumb from '../components/BreadCrumb'
import watch from "../images/watch.jpg"
import { MdDeleteForever } from "react-icons/md";
import { Link } from 'react-router-dom';
import Container from '../components/Container';

const Cart = () => {
  return (
    <>
      <Meta title="Cart" />
      <BreadCrumb title="Cart"/>
      <Container class1='cart-wrapper home-wrapper-2 py-5'>
        <div className='row'>
            <div className='col-12'>
                <div className='cart-header d-flex justify-content-between align-items-center'>
                    <h4 className='cart-col-1'>Product</h4>
                    <h4 className='cart-col-2'>Price</h4>
                    <h4 className='cart-col-3'>Quantity</h4>
                    <h4 className='cart-col-4'>Total</h4>
                </div>
                <div className='cart-data mb-2 d-flex justify-content-between align-items-center'>
                    <div className='cart-col-1 gap-15 d-flex align-items-center'>
                        <div className='w-25'>
                            <img src={watch} className='img-fluid' alt='watch'/>
                        </div>
                        <div className='w-75'>
                            <p>Laravel is a free and open-source PHP web framework laptop</p>
                            <p>Size : XL</p>
                            <p>Color : XL</p>
                        </div>
                    </div>
                    <div className='cart-col-2'>
                        <h5 className='price'>$ 100.00</h5>
                    </div>
                    <div className='cart-col-3 d-flex align-items-center gap-15'>
                        <div>
                            <input type='number' className='form-control' name='' id='' min={1} max={20} defaultValue={1}/>
                        </div>
                        <div>
                            <MdDeleteForever className='fs-3 text-danger' />
                        </div>
                    </div>
                    <div className='cart-col-4'>
                        <h5 className='price'>$ 100.00</h5>
                    </div>
                </div>
                <div className='cart-data mb-2 d-flex justify-content-between align-items-center'>
                    <div className='cart-col-1 gap-15 d-flex align-items-center'>
                        <div className='w-25'>
                            <img src={watch} className='img-fluid' alt='watch'/>
                        </div>
                        <div className='w-75'>
                            <p>Laravel is a free and open-source PHP web framework laptop</p>
                            <p>Size : XL</p>
                            <p>Color : XL</p>
                        </div>
                    </div>
                    <div className='cart-col-2'>
                        <h5 className='price'>$ 100.00</h5>
                    </div>
                    <div className='cart-col-3 d-flex align-items-center gap-15'>
                        <div>
                            <input type='number' className='form-control' name='' id='' min={1} max={20} defaultValue={1}/>
                        </div>
                        <div>
                            <MdDeleteForever className='fs-3 text-danger' />
                        </div>
                    </div>
                    <div className='cart-col-4'>
                        <h5 className='price'>$ 100.00</h5>
                    </div>
                </div>
            </div>
            <div className='col-12 py-2 mt-4'>
                <Link to="/store" className='button'>Continue To Shipping</Link>
            </div>
            <div className='col-12 py-2 mt-4'>
                <div className='d-flex flex-column align-items-center justify-content-center'>
                    <h4>SubTotal : $ 100.00</h4>
                    <p>Texes and Shopping Calculated at Checkout</p>
                    <div>
                        <Link to="/checkout" className='button'>Checkout</Link>
                    </div>
                </div>
            </div>
        </div>
      </Container>
    </>
  )
}

export default Cart
